...
import 'package:mobile/entity/model/protype.dart';
...
class ServiceController extends GetxController {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (response_body != null && response_body['token'] != null) {
          ...
          isloading.value = false;
          UI.toast(text: "ប្រតិបត្តិការបរាជ័យ", isSuccess: false);
  ...
  Future<List<Cashier>> fetchCashier() async {
    final box = GetStorage();
    final token = box.read('token');
    var response = await http.get(
      Uri.parse("${mainUrlApi}admin/dashboard/cashier"),
      headers: {
        'Accept': 'application/json',
        'Authorization': 'Bearer $token',
      },
  ...
  Future<StatisticProductType> fetchProductTypeStatistics() async {
    final box = GetStorage();
    final token = box.read('token');
    var response = await http.get(
      Uri.parse("${mainUrlApi}admin/dashboard/product-type"),
      headers: {
        'Accept': 'application/json',
        'Authorization': 'Bearer $token',
      },
  ...
  Future<StatisticSales> fetchSaleStatistics() async {
    final box = GetStorage();
    final token = box.read('token');
    var response = await http.get(
      Uri.parse("${mainUrlApi}admin/dashboard/data-sale"),
      headers: {
        'Accept': 'application/json',
        'Authorization': 'Bearer $token',
      },
  ...
  Future<void> createUser(String name, List<int> roleIds, String phone,
    ...
    try {
      ...
      if (response.statusCode == 400) {
        ...
        UI.toast(text: 'មិនអាចបង្កើតអ្នកប្រើប្រាស់', isSuccess: false);
        var data = jsonDecode(response.body);
        log("${data['message']}");
      } else if (response.statusCode == 201) {
        log('អ្នកប្រើប្រាស់ត្រូវបានបង្កើតដោយជោគជ័យ: ${response.body}');
        UI.toast(text: 'អ្នកប្រើប្រាស់ត្រូវបានបង្កើតដោយជោគជ័យ');
      } else {
        log('Failed to create អ្នកប្រើប្រាស់: ${response.body}');
        UI.toast(text: 'Failed to create អ្នកប្រើប្រាស់', isSuccess: false);
      }
    } catch (e, stackTrace) {
      log('Error creating អ្នកប្រើប្រាស់');
  ...
  Future<void> updateUser(String name, List<int> roleIds, String phone,
    ...
    try {
      ...
      if (response.statusCode == 400) {
        ...
        log("${roleIds.toString()}");
        UI.toast(text: 'មិនអាចកែប្រែព័ត៍មាន', isSuccess: false);
        var data = jsonDecode(response.body);
        log("${data['message']}");
      } else if (response.statusCode == 201) {
        log('អ្នកប្រើប្រាស់ត្រូវបានកែប្រែដោយជោគជ័យ: ${response.body}');
        UI.toast(text: 'អ្នកប្រើប្រាស់ត្រូវបានកែប្រែដោយជោគជ័យ');
      } else {
        log('អ្នកប្រើប្រាស់: ${response.body}');
        UI.toast(
          text: 'កែប្រែព័ត៍មានអ្នកប្រើប្រាស់បានជោគជ័យ',
        );
      }
    } catch (e, stackTrace) {
      log('Error creating អ្នកប្រើប្រាស់');
  ...
  Future<bool> createProductType(String productName) async {
    try {
      ...
      if (response.statusCode == 400) {
        ...
        UI.toast(text: 'មិនអាចបង្កើតផលិតផល', isSuccess: false);
        return false;
      }
  ...
  Future<bool> daleteProductType(int id) async {
    try {
      ...
      if (response.statusCode == 400) {
        UI.toast(text: "មិនអាចលុបប្រភេទផលិតផលដែលមានផលិតផល", isSuccess: false);
        return false;
      }
  ...
  static Future<void> createProduct({
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        UI.toast(text: 'បង្កើតផលិតផលបានជោគជ័យ'); // Success toast
  ...
  Future<void> updateProduct(String productName, String code, String type,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        print('Product created successfully: ${response.body}');
        UI.toast(text: 'ផលិតផលបានកែប្រែដោយជោគជ័យ'); // Success toast
      } else {
        print('Failed to create product: ${response.body}');

        UI.toast(
            text: 'ការកែប្រែផលិតផលបានបរាជ័យ',
